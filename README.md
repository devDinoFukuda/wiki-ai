...
| `wiki-ai ask "<pergunta>" --repo <repo>` | Consultar a base já investigada | Roda `Answerer` sobre o `KnowledgeRepository`; bloqueia se a base estiver vazia ou se nenhuma afirmação sobreviveu à validação | `status` (`ok`/`partial`/`blocked`), `answer`, `mode`, `evidence_ids`, `entity_ids`, `unresolved`, `action` quando não é `ok` | 0/1/2 | `status: "ok"` com `answer` preenchido |
...
| `ask` | `status` | `ok`, `partial`, `blocked` | `app/api.py::ASK_STATUS_BY_ANSWER` (mapeia `publishing/answer.py::AnswerStatus`: `answered`→`ok`, `partial`→`partial`, `blocked`→`blocked`) |
...
| `ask` com `status: partial` | Parte das afirmações do agente foi descartada na validação; as descartadas aparecem em `unresolved` | Ler `unresolved`; ampliar a evidência (novo `analyze`/`ingest`) antes de confiar na resposta |
| `ask` com `status: blocked` | Nenhuma afirmação do agente passou na validação (`reason: no_validated_claim`); a resposta devolvida é o fallback determinístico | Ler `unresolved`/`action`; a base não sustenta a pergunta — rodar `analyze`/`ingest` para trazer evidência |

`up_to_date` (`reason` de `analyze`) só é retornado quando o `analysis_status` da última execução é `complete` **e** o snapshot, o objective e a versão do contrato de análise batem (`app/session.py::AnalysisState.is_current_for(digest, objective_hash)`). A chave de análise é `snapshot_digest` + `objective_hash` (SHA-256 do objective normalizado; o `DEFAULT_OBJECTIVE` tem hash próprio) + `ANALYSIS_CONTRACT_VERSION`, persistidos em `analysis_state.json` como `analyzed_objective_hash` e `contract_version`. Consequências: o mesmo snapshot com um `--objective` diferente **roda uma nova investigação**; a atualização incremental só roda quando a análise anterior é `complete` **para o mesmo objective** (`app/api.py::_settled_baseline`) — se a baseline for `partial`/`blocked`/`failed`, ou o objective for outro, o `analyze` roda a investigação **completa** sobre o snapshot novo, porque um update incremental sobre uma baseline não concluída fecharia como `complete` sem nunca ter fechado a frontier aberta; um estado gravado antes desses campos, ou com `contract_version` antiga, nunca é considerado atualizado — **subir `ANALYSIS_CONTRACT_VERSION` (`app/session.py`, hoje `"2"`) invalida toda baseline gravada por uma versão anterior: o próximo `analyze` roda a investigação completa, nunca o update incremental (`app/api.py::_settled_baseline` exige `state.contract_version == ANALYSIS_CONTRACT_VERSION`), e regrava o estado com a versão nova**; e uma análise `partial`/`blocked`/`failed` continua não marcando o repositório como atualizado.
