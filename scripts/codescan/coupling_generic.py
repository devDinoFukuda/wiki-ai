"""Motor GENÉRICO multi-linguagem de acoplamento — zonas de design (métricas
de componente de Martin), determinísticas.

Fallback do dispatcher `coupling.py`: usado quando o repo não tem Java (ou
quando o motor Java roda mas nenhuma classe sobrevive às exclusões). Cobre
qualquer linguagem reconhecida por `surface.py::LANGUAGES` a partir de um
único parser de import por regex-família — mais raso que `coupling_java.py`
(que faz parse do corpo de cada classe Java), mas funciona em qualquer stack.

Ce (eferente), Ca (aferente), Instabilidade I = Ce/(Ce+Ca), Abstração A,
Distância D = |A + I − 1| e a Zona resultante — por MÓDULO do surface, não
por classe/arquivo (o motor Java, por ser específico da linguagem, também
oferece a granularidade por classe).
